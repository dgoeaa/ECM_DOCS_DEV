...
//
// Same paths, same request bodies and same response fields as proxy/src/intake.js and the
// `/documents/scan` route, so document-portal/js/core.js and core/scan-intake-service.js
// need no change to talk to this.
//
// What is deliberately NOT reproduced: token validation, role authorization and identity
// stripping. Those are the proxy's reason to exist and they need an Entra tenant. This file
// accepts what it is given and records it. It is a development stand-in, and the banner in
// dev-server.mjs says so on every start.
//
...
export function handleIntake(path, method, body, store, { headers = {} } = {}) {
  ...
  if (route === 'intake/verify-confirm' && method === 'POST') {
    ...
    // One reason for every failure, exactly as the proxy does: telling "no challenge",
