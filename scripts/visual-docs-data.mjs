...
function quality() {
  ...
  return {
    ...
    ciSteps: steps,
    testFiles,
    proxyTests: fs.readdirSync(path.join(ROOT, 'proxy/test')).filter(f => f.endsWith('.mjs')).sort(),
...
const channels = [
  { id: 'A', label: 'Public portal', origin: 'External submitter, no account',
    entry: 'document-portal/submit.html', path: 'proxy /intake/submission', channelValue: 'Portal',
  ...
  { id: 'C', label: 'Scan / physical counter', origin: 'Registry counter clerk',
    entry: 'modules/scan-intake.js', path: 'proxy /documents/scan', channelValue: 'Document',
...
const zones = [
  ...
  { id: 'enforcement', label: 'Enforcement', tone: 'enforce',
  ...
  { id: 'record', label: 'Systems of record', tone: 'record',
    rule: 'Reachable from the enforcement zone only. Private endpoint / IP restricted.',
...
const data = {
  ...
  taxonomy: {
    ...
    sequenceWidth: refMinter.SEQUENCE_WIDTH,
    filenamePolicy: {
...
if (process.argv.includes('--write')) {
  ...
  console.log(
    ...
    `${data.backend.modules.length} proxy modules · ${data.dataModel.listCount} lists / ${data.dataModel.fieldCount} fields · ` +
