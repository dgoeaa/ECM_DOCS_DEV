...
(function () {
  ...
  S({
    ...
    render: function () {
      ...
      out += '<div class="callout" data-aud="exec arch dev ops"><div class="lbl">In one paragraph</div><p>' + h(pr.summary) + '</p>' +
        ...
        'One component does, and everything passes through it. That single sentence explains the trust zones, the proxy, the upload design and half the roadmap.</p></div>';
      ...
      out += '<div class="grid g2" data-aud="exec arch dev ops">' +
        [['Executive briefing', 'Sections 1–3, 13 and 17. The rule, the zones, the document\'s journey, the security posture and what remains to decide. Roughly twelve minutes at a walk.'],
         ['Architecture review', 'Sections 2–5, 9–10 and 14. Zones, layering, workspace topology, the enforcement tier and the lifecycle that governs every record.'],
  ...
  S({
    ...
    render: function () {
      ...
      out += '<div class="grid g3">' +
        kpi(P.taxonomy.referencePrefix + '-YYYY-' + '0'.repeat(P.taxonomy.sequenceWidth), 'reference format', 'minted server-side, never client-chosen') +
        kpi(P.taxonomy.sequenceWidth, 'sequence digits', 'zero-padded, sortable, collision-checked') +
      ...
      out += '<p data-aud="arch dev"><strong>Why the sequence is not a variable in memory.</strong> It was, and two processes both minted <code>' +
        h(P.taxonomy.referencePrefix) + '-YYYY-000001</code>: on a server that needed a restart, on an edge isolate it needs only a cold start, which happens routinely and invisibly. Two citizens then hold a receipt for the same reference and the register contains it twice. For a records system that is not a rough edge — it is the register being wrong. It now lives in a Durable Object, single-threaded with strongly consistent storage, because a sequence needs an <em>atomic</em> read-modify-write and eventually-consistent storage cannot provide one.</p>';
  ...
  S({
    id: 'backend', part: 'Back end', title: 'The authenticating proxy', aud: 'arch dev ops exec',
    eyebrow: 'Sheet 8 · the enforcement tier',
    note: 'The component that makes governance <em>enforced</em> rather than advisory. Until it is deployed, every control in both clients is a user-experience affordance: a browser can decline to send a request, but it can never prevent one.',
    render: function () {
      var b = P.backend, out = figProxy();
      out += '<div class="grid g4">' +
  ...
  S({
    ...
    note: '<code>/intake/*</code> is the only route through the proxy that does not require a token, because a citizen writing to NITDA has no account and should not need one. Everything about its design follows from being unauthenticated.',
    render: function () {
      var out = '';
      out += '<div class="grid g2">' + [
  ...
  S({
    ...
    render: function () {
      ...
      out += '<p>One matrix, two consumers: the client renders navigation from it and the proxy authorizes against it. Not a copy — <code>proxy/src/authorize.js</code> imports it.</p>';
      ...
      out += '<div class="callout crit" data-aud="exec arch ops"><div class="lbl">Open, and it must not be softened</div>' +
        '<p>Signed Power Automate trigger URLs were published in client-delivered files. <strong>Removing a file revokes nothing, and neither does rewriting history.</strong> Every affected signature must be rotated in Power Automate. The ratchet at <code>tests/secrets-baseline.txt</code> tracks the known set and may only shrink; ' +
        (s.secretsBaseline.length
          ? 'it currently lists <strong>' + s.secretsBaseline.length + '</strong> entries.'
          : 'it is currently empty at this commit — which records the tracked files, not the rotation itself.') +
        ' Rotation comes first; the deployment guide requires regenerating every trigger before its URL is placed in a Worker secret, or the exercise moves a leaked credential rather than replacing it.</p></div>';
      return out;
    }
  });
  ...
  S({
    ...
    note: 'Cloudflare at the edge in front of the existing Power Automate flows. The flows keep their signed trigger URLs; those URLs live in Worker secrets and are reachable only from there. A browser gets a token and the Worker, never a flow URL.',
    render: function () {
  ...
  S({
    ...
    render: function () {
      ...
      out += tbl(['Item', 'Why it matters', 'What closes it'], [
        ...
        ['<b>Decommission the dev/pilot flows</b>', '25 workflows carry a published signed URL; 17 are referenced by nothing, and 6 carry two signatures because a trigger was regenerated without the old one being revoked.', '<code>docs/cutover/FLOW_DECOMMISSION_INVENTORY.md</code>'],
        ['<b>Rendered-appearance regression coverage</b>', 'Nothing beyond the smoke suite\'s theme check protects the cascade, and <code>styles/index.css</code> documents measured, accepted debt in its overrides layer.', 'A visual-regression pass over the certified viewport matrix.']
      ]);
      out += '<div class="callout" data-aud="exec"><div class="lbl">The one-line summary for a decision meeting</div>' +
        '<p>The platform is built and running; the component that makes its governance real is written, tested and not yet deployed. The remaining work is deployment and credential rotation — configuration events with a defined walkthrough — not new development.</p></div>';
      return out;
    }
  });
  ...
  S({
    ...
    render: function () {
      var terms = [
        ...
        ['Reference', 'The registry identifier issued to a correspondence: <code>' + h(P.taxonomy.referencePrefix) + '-YYYY-' + '0'.repeat(P.taxonomy.sequenceWidth) + '</code>. Minted server-side, never chosen by a client.'],
  ...
  document.getElementById('railFoot').innerHTML =
    ...
    n(P.backend.totalLines) + ' proxy lines<br>' +
